# код взят из: https://habr.com/ru/sandbox/145868/ и немного доработан
from django import template
import string

#словарь с цензурируемыми словами
words = ["банан", "помидор", "рука", "робот"]
    ...
    for word in words:
        ...
        for part in range(len(phrase)):
            ...
            if distance(fragment, word) <= len(word)*0.25:
                #Если они равны, заменяем фрагмент в исходном тексте на ***
                print("Найдено", word, "\nПохоже на", fragment)
                text = text.replace(text[part: part+len(word)], '*' * len(word))
    return text
